...
export default function PromptathonSection() {
    return (<section id="promptathon">
      <div className="section-shell">
        <SectionTitle eyebrow="Prompt-A-Thon" title={promptathon.title} subtitle={promptathon.subtitle}/>

        <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {promptathon.details.map(function (detail) { return (<article key={detail.label} className="card p-4">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <article className="card">
            ...
            <ul className="mt-3 space-y-2 text-slate-600">
              {promptathon.skills.map(function (skill) { return (<li key={skill} className="flex items-start gap-2">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-3">
          <article className="card">
            ...
            <ul className="mt-3 list-disc space-y-2 pl-5 text-sm text-slate-600">
              {promptathon.bringItems.map(function (item) { return (<li key={item}>{item}</li>); })}
            </ul>
          </article>
          ...
          <article className="card">
            ...
            <ul className="mt-3 list-disc space-y-2 pl-5 text-sm text-slate-600">
              {promptathon.format.map(function (item) { return (<li key={item}>{item}</li>); })}
            </ul>
          </article>
          ...
          <article className="card">
            ...
            <ul className="mt-3 list-disc space-y-2 pl-5 text-sm text-slate-600">
              {promptathon.notes.map(function (item) { return (<li key={item}>{item}</li>); })}
            </ul>
          </article>
        </div>
      </div>
