    ...
    <section id="ai-bits">
      <div className="section-shell">
        <SectionTitle
          ...
          title="Recent Daily Bits and full explainers"
          subtitle="Quick summaries up top, then detailed reading cards below so every call-to-action leads somewhere real."
        />
        ...
        <div className="mt-10 space-y-4">
          {aiBits.map((bit) => (
            <article id={`ai-bit-${bit.id}`} key={bit.id} className="card scroll-mt-24">
              <div className="mb-3 flex flex-wrap gap-2">
                {bit.tags.map((tag) => (
                  <span key={tag} className="pill">
                    {tag}
