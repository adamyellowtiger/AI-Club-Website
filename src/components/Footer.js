...
export default function Footer() {
    return (<footer className="border-t border-blue-100 bg-white">
      <div className="section-shell grid gap-8 md:grid-cols-3">
        ...
        <div>
          ...
          <ul className="mt-2 space-y-1 text-sm">
            {navLinks.slice(0, 6).map(function (link) { return (<li key={link.href}><a href={link.href} className="text-slate-600 hover:text-blue-700">{link.label}</a></li>); })}
          </ul>
        </div>
        <div>
          <h4 className="font-semibold text-slate-900">Contact</h4>
          <ul className="mt-2 space-y-1 text-sm">
            {joinLinks.map(function (link) { return (<li key={link.label}><a href={link.href} className="text-slate-600 hover:text-blue-700">{link.label}</a></li>); })}
          </ul>
